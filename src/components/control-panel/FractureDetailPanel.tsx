...
import type { MeshRole, Robot, RobotModel, RobotStatus, ScenarioType } from '../../types';
...
export function FractureDetailPanel() {
  ...
  if (!selectedFracture) {
    const emptyStateMap: Record<string, { icon: string; hint: Record<Locale, string>; label: Record<Locale, string> }> = {
      coal: { icon: '⛏', hint: { 'zh-CN': '点击 3D 场景中的机器人、监控或位置', 'en-US': 'Click a robot or fracture in the 3D scene' }, label: { 'zh-CN': '查看设备状态、瓦斯、应力、渗透率等参数', 'en-US': 'Inspect robot status, CH4, stress, and permeability readings' } },
